...
import 'custom_just_audio_media_kit.dart';
...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    _player = Player(
      ...
      pitch: CustomJustAudioMediaKit.pitch,
      protocolWhitelist: CustomJustAudioMediaKit.protocolWhitelist,
      title: CustomJustAudioMediaKit.title,
      bufferSize: CustomJustAudioMediaKit.bufferSize,
      logLevel: CustomJustAudioMediaKit.mpvLogLevel,
      ready: () => _readyCompleter.complete(),
    ));

    if (CustomJustAudioMediaKit.prefetchPlaylist) {
  ...
  Future<LoadResponse> load(LoadRequest request) async {
    ...
    _updatePlaybackEvent();
    // We do not await _loadCompleter?.future here because it can cause just_audio to hang indefinitely
    // if the buffering stream doesn't fire exactly as expected.
